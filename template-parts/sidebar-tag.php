<?php
/**
 * Template part for displaying sidebar content in tag.php
 *
...
?>
		...
		<div class="card bg-white border-dark mb-3 rounded">
		    <div class="card-body">
			<h3 class="card-title"><?php esc_html_e( 'Tests in', 'gustumexlibro' ); echo ' '.ucfirst($tag_name); ?></h3>
			<p class="card-text text-justify">
			    <?php
			    $stats_args = array(
				'tag_id' 	 => $tag_id
			    );
			    $tag_stats = new WP_Query( $stats_args );
			    printf( // WPCS: XSS OK.
				/* translators: 1: tests count number, 2: tag. */
				_nx( 'Tu Peux Pas Zeste tasted and tested <span class="font-weight-bold">%1$s</span> lemon meringue product for the tag &laquo; %2$s &raquo;.', 'Tu Peux Pas Zeste tasted and tested <span class="font-weight-bold">%1$s</span> lemon meringue products during the tag &laquo; %2$s &raquo;.', $tag_stats->post_count, 'number of tests for a tag', 'gustumexlibro' ),
		...
		<div class="card bg-white border-dark mb-3 rounded">
		    <div class="card-body">
			<h3 class="card-title"><?php esc_html_e( 'Top 20 in', 'gustumexlibro' ); echo ' '.ucfirst($tag_name); ?></h3>
