<?php
/**
 * Template part for displaying sidebar content in palmares.php
 *
...
?>
		...
		<div class="card bg-white border-dark mb-3 rounded">
		    <div class="card-body">
			<h3 class="card-title"><?php esc_html_e( 'Tests in', 'gustumexlibro' ); echo ' ' . $palmares_date; ?></h3>
			<p class="card-text text-justify">
			    ...
			    $palmares_stats = new WP_Query( $stats_args );
			    printf( // WPCS: XSS OK.
				/* translators: 1: number of tests for an year, 2: year. */
				_nx( 'Tu Peux Pas Zeste tasted and tested <span class="font-weight-bold">%1$s</span> lemon meringue product during the year %2$s.', 'Tu Peux Pas Zeste tasted and tested <span class="font-weight-bold">%1$s</span> lemon meringue products during the year %2$s.', $palmares_stats->post_count, 'number of tests for an year', 'gustumexlibro' ),
				$palmares_stats->post_count,
				$palmares_date
			    );
			    ?>
			</p>
		...
		<div class="card bg-white border-dark mb-3 rounded">
		    <div class="card-body">
			<h3 class="card-title"><?php esc_html_e( 'Top 20 in', 'gustumexlibro' ); echo ' ' . $palmares_date; ?></h3>
		...
		<div class="card bg-white border-dark mb-3 rounded">
		    <div class="card-body">
			<h3 class="card-title"><?php esc_html_e( 'Tags List', 'gustumexlibro' ); ?></h3>
			<p class="card-text">
			    <?php
			    $tags = get_tags();
			    foreach ( $tags as $tag ) {
				$tag_link = get_tag_link( $tag->term_id );
				echo "<a href='{$tag_link}' title='{$tag->name} Tag' class='{$tag->slug} text-dark nounderline'>{$tag->name}</a>&nbsp;";
			    }
			    ?>
			</p>
		    </div><!-- .card-body -->
